...
def get_person_list(session):
    ...
    try:
        ...
        all_serial_code = [x['SERIAL_CODE'] for x in page_data]
        print('个人档案列表一共有: %s' % len(all_serial_code))
        return all_serial_code
...
def get_insert_all_person_detail():
    ...
    all_serial_code = get_person_list(session=SESSION_xjh)
